...
srvd_boolean_t srvd_protocol_packet_field_add(srvd_protocol_packet_t *, srvd_protocol_type_t,
                                              uint16_t, const void *);
srvd_boolean_t srvd_protocol_packet_field_inject(srvd_protocol_packet_t *, srvd_protocol_type_t,
                                                 uint16_t, const void *);
srvd_boolean_t srvd_protocol_packet_field_get(const srvd_protocol_packet_t *,
                                              uint16_t, srvd_protocol_packet_field_t **);
...
static inline
size_t srvd_protocol_packet_field_size(const srvd_protocol_packet_t *packet) {
  srvd_protocol_packet_field_t *i;
  size_t size = 0;

  SRVD_RETURN_VALUE_UNLESS(packet, 0);
...
static inline
srvd_boolean_t srvd_protocol_packet_field_add_uint32(srvd_protocol_packet_t *packet,
                                                     srvd_protocol_type_t type, uint32_t data) {
  uint32_t v = htonl(data);
  return srvd_protocol_packet_field_add(packet, type, sizeof(uint32_t), &v);
}

static inline
srvd_boolean_t srvd_protocol_packet_field_add_uint16(srvd_protocol_packet_t *packet,
                                                     srvd_protocol_type_t type, uint16_t data) {
  uint16_t v = htons(data);
  return srvd_protocol_packet_field_add(packet, type, sizeof(uint16_t), &v);
}

static inline
srvd_boolean_t srvd_protocol_packet_field_add_uint8(srvd_protocol_packet_t *packet,
                                                    srvd_protocol_type_t type, uint8_t data) {
  return srvd_protocol_packet_field_add(packet, type, sizeof(uint8_t), &data);
}

static inline
srvd_boolean_t srvd_protocol_packet_field_inject_uint32(srvd_protocol_packet_t *packet,
                                                        srvd_protocol_type_t type, uint32_t data) {
  uint32_t v = htonl(data);
  return srvd_protocol_packet_field_inject(packet, type, sizeof(uint32_t), &v);
}

static inline
srvd_boolean_t srvd_protocol_packet_field_inject_uint16(srvd_protocol_packet_t *packet,
                                                        srvd_protocol_type_t type, uint16_t data) {
  uint16_t v = htons(data);
  return srvd_protocol_packet_field_inject(packet, type, sizeof(uint16_t), &v);
}

static inline
srvd_boolean_t srvd_protocol_packet_field_inject_uint8(srvd_protocol_packet_t *packet,
                                                       srvd_protocol_type_t type, uint8_t data) {
  return srvd_protocol_packet_field_inject(packet, type, sizeof(uint8_t), &data);
}

/* Value getters (automatically preserve byte order). */
static inline
srvd_boolean_t srvd_protocol_packet_field_get_uint32(srvd_protocol_packet_field_t *field,
                                                     uint32_t *data) {
  SRVD_RETURN_FALSE_UNLESS(field);
  SRVD_RETURN_FALSE_UNLESS(field->size == sizeof(uint32_t));
...
static inline
srvd_boolean_t srvd_protocol_packet_field_get_uint16(srvd_protocol_packet_field_t *field,
                                                     uint16_t *data) {
  SRVD_RETURN_FALSE_UNLESS(field);
  SRVD_RETURN_FALSE_UNLESS(field->size == sizeof(uint16_t));
  SRVD_RETURN_FALSE_UNLESS(data);
...
static inline
srvd_boolean_t srvd_protocol_packet_field_get_uint8(srvd_protocol_packet_field_t *field,
                                                    uint8_t *data) {
  SRVD_RETURN_FALSE_UNLESS(field);
  SRVD_RETURN_FALSE_UNLESS(field->size == sizeof(uint8_t));
  SRVD_RETURN_FALSE_UNLESS(data);
